...
class PositionEmbeddingLearned(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        x_emb = self.col_embed(i)                # [W, Cpos]
        y_emb = self.row_embed(j)                # [H, Cpos]
        B = x.shape[0]
        pos = torch.cat([
            x_emb.unsqueeze(0).repeat(h, 1, 1),  # [H,W,Cpos]
            y_emb.unsqueeze(1).repeat(1, w, 1),  # [H,W,Cpos]
        ], dim=-1).unsqueeze(2).repeat(1, 1, B, 1)   # [H,W,B,2*Cpos]
        return pos
