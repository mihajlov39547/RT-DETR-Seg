...
def get_config(size, use_registers):
    ...
    if size not in config_dict:
        raise ValueError(
            f"Unsupported DINOv2 size '{size}'. Supported: {list(config_dict.keys())}"
        )
...
class DinoV2(nn.Module):
    def __init__(self,
            ...
            patch_size=14,
            num_windows=4,
            positional_encoding_size: int | None = None,  # <-- changed
        ):
        super().__init__()
            ...
            if positional_encoding_size is not None:
                ...
                if implied_resolution != dino_config["image_size"]:
                    print(
                        ...
                        "which means we're not loading DINOv2 backbone weights. "
