...
def resize(image, target, size, max_size=None):
    ...
    if "masks" in target:
        target['masks'] = F_torch.interpolate(
...
class SquareResize(object):
    ...
    def __call__(self, img, target=None):
        ...
        rescaled_img = F.resize(img, (size, size))  # (H,W)
        w_new, h_new = rescaled_img.size          # PIL: (W,H)
        if target is None:
            return rescaled_img, None

        # scales from original → new
        ratio_w = w_new / img.size[0]
        ratio_h = h_new / img.size[1]
        ...
        if "area" in target:
            area = target["area"]
            target["area"] = area * (ratio_w * ratio_h)

        # update logical size (H,W)
        target["size"] = torch.tensor([h_new, w_new])

        # --- NEW: resize masks (nearest) ---
        if "masks" in target:
            target["masks"] = F_torch.interpolate(
                target["masks"][:, None].float(), (h_new, w_new), mode="nearest"
            )[:, 0] > 0.5
...
class Pad(object):
    ...
    def __call__(self, im, target):
        ...
        if self.size:
            h, w = self.size
            assert im_h <= h and im_w <= w, "target size must be >= input size"
        else:
        ...
        # decide offsets per mode
        if self.pad_mode == -1:
            assert self.offsets is not None
            offset_x, offset_y = self.offsets
        elif self.pad_mode == 0:    # right+bottom
            offset_y, offset_x = 0, 0
        elif self.pad_mode == 1:    # centered
        ...
        # --- NEW: pad masks consistently with image placement ---
        if "masks" in target:
            masks = target["masks"]  # (N,H,W) tensor
            n, mh, mw = masks.shape if masks.numel() > 0 else (0, im_h, im_w)
            if self.pad_mode == 0:
                # right+bottom only
                pad_w = w - im_w
                pad_h = h - im_h
                target["masks"] = torch.nn.functional.pad(
                    masks, (0, pad_w, 0, pad_h), value=0
                )
            else:
                # place at (offset_y, offset_x) on a larger canvas
                new_masks = torch.zeros((n, h, w), dtype=masks.dtype)
                if n > 0:
                    new_masks[:, offset_y:offset_y + im_h, offset_x:offset_x + im_w] = masks
