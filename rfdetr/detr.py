...
"""
dert.py
This module implements the RF-DETR class, which provides methods for training,
running inference, optimizing models for deployment, and exporting models to ONNX format.
It also includes specific configurations for different RF-DETR model sizes.
"""
import json
import os
...
class RFDETR:
    ...
    def optimize_for_inference(self, compile=True, batch_size=1, dtype=torch.float32):
        ...
        self.model.inference_model = deepcopy(self.model.model).eval()
        self.model.inference_model.export()
        # put on the same device & dtype as inputs we’ll feed later
        self.model.inference_model = self.model.inference_model.to(device=self.model.device, dtype=dtype)
        ...
        if compile:
            ...
            self.model.inference_model = torch.jit.trace(self.model.inference_model, example)
    ...
    def train_from_config(self, config: TrainConfig, **kwargs):
        ...
        # --- Freeze intent & mask data guard ---
        freeze_intent = bool(getattr(config, "masks", False) and getattr(config, "frozen_weights", None))
        if getattr(config, "masks", False):
            has_any_segm = any(a.get("segmentation") for a in anns.get("annotations", []))
            if not has_any_segm:
                raise ValueError(
                    "masks=True but no 'segmentation' found in COCO annotations. "
                    "Use instance annotations (polygons/RLE) for training the mask head."
                )

        # --- Current detector head dim vs. dataset classes ---
        desired_out_dim = num_classes + 1  # includes background
        def _current_out_dim(model):
            if hasattr(model, "class_embed"):
                return getattr(model.class_embed, "out_features", None)
            if hasattr(model, "detr") and hasattr(model.detr, "class_embed"):
                return getattr(model.detr.class_embed, "out_features", None)
            return None

        cur_out_dim = _current_out_dim(self.model)

        # When freezing detector (stage-2), DO NOT reinit the head. Enforce exact match instead.
        if freeze_intent:
            if cur_out_dim is None:
                raise RuntimeError("Frozen training requested but class head not found on model.")
            if cur_out_dim != desired_out_dim:
                raise ValueError(
                    f"Frozen detector head out_dim ({cur_out_dim}) != dataset ({desired_out_dim}). "
                    f"Train the detector first on this dataset (same classes), then freeze for masks."
                )
        else:
            # Not freezing → allow reinit if mismatch
            if cur_out_dim is not None and cur_out_dim != desired_out_dim:
                logger.warning(
                    f"num_classes mismatch: model head={cur_out_dim-1}, dataset={num_classes}. "
                    f"Reinitializing detection head to {num_classes} classes."
                )
                target = self.model if hasattr(self.model, "reinitialize_detection_head") \
                        else getattr(self.model, "detr", None)
                if target and hasattr(target, "reinitialize_detection_head"):
                    # pass classes without bg; helper will add +1
                    target.reinitialize_detection_head(num_classes, include_background=True)
                else:
                    logger.warning("Cannot reinitialize head: method not found; continuing as-is.")

        # --- Merge configs (model < train < kwargs), keep num_classes from dataset ---
        train_config = config.dict()
        model_config = self.model_config.dict()

        # remove fields that must be driven by data or are runtime-only
        ...
        # prefer train_config over model_config; and explicit kwargs over both
        for k in list(train_config.keys()):
            model_config.pop(k, None)
        for k in list(kwargs.keys()):
            train_config.pop(k, None)
            model_config.pop(k, None)
        ...
        all_kwargs = {
            ...
            **train_config,
            **kwargs,
            "num_classes": num_classes,          # ensure criterion uses dataset classes
        ...
        **kwargs,
        ...
        if not self._is_optimized_for_inference and not self._has_warned_about_not_being_optimized_for_inference:
            logger.warning(
                ...
                "Call model.optimize_for_inference() to compile."
            )
            self._has_warned_about_not_being_optimized_for_inference = True

        # ensure eval every call
